...
> Current route: `phase-01-p1-r2-router-r1` authorizes only one independent P1.R2
> semantic/auth/error/cursor/kernel-size review of canonical
> `666042037a9c91df572b1d8274bf6024f8d00f40`. The reviewer uses `gpt-5.6-sol`,
> `xhigh`, and `serviceTier: "default"`; Fast is not authorized. This seven-path router launches
> nothing and ends `HOLD`. Reviewer dependencies are broker-materialized offline; the reviewer must
> not install dependencies.

Phase 0 and accepted Phase 1 history remain frozen. PR #252's conflict gate and P1.1D are complete
and accepted. Their accepted true merge is canonical, clean, and remote-equal at
`666042037a9c91df572b1d8274bf6024f8d00f40`, with ordered parents
`c3135d40c6e70e4b2ddc905dc815407397197634` and
`3b48f9391b4bff1d82bc85ef01a2d5e0e5b50e95`. The old
...
router is accepted, policy-integrated, and pushed.
...
Do not recursively inspect research, rejected job state, or unrelated product/test paths. Do not
fetch, stage, commit, merge, push, launch an app/runtime/team, access a real project, or substitute a
moving reference for the stored canonical SHA.

## Current review contract

Exactly one fresh independent reviewer may review the canonical shared hosted kernel and the
team-lifecycle list contract/use case. Its only writable paths are:
