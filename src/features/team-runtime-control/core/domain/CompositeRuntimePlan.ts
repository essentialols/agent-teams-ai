import type { CompositeRuntimePlan, CompositeRuntimePlanHash } from '../../contracts';

export type CompositeRuntimePlanHashBody = Omit<CompositeRuntimePlan, 'planHash'>;
...
function canonicalJson(value: unknown): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) {
      throw new TypeError('runtime-plan-hash-non-finite-number');
    }
    return JSON.stringify(value);
  }
  ...
  throw new TypeError('runtime-plan-hash-value-unsupported');
}
