...
export class TeamProvisioningService {
  ...
  private async buildTeamAgentRuntimeSnapshot(
    ...
    return buildTeamAgentRuntimeSnapshotHelper({
      ...
      agentRuntimeResourceHistory: this.agentRuntimeResourceHistory,
  ...
  private async runProviderOneShotDiagnostic(
    ...
    for (let attempt = 1; attempt <= PREFLIGHT_AUTH_MAX_RETRIES; attempt++) {
      ...
      const isAuthFailure = isAuthFailureWarning(combinedOutput, 'probe');
      ...
      if (isAuthFailure || pingProbe.exitCode !== 0) {
        const normalizedOutput =
          normalizeApiRetryErrorMessage(combinedOutput) || combinedOutput.trim();
