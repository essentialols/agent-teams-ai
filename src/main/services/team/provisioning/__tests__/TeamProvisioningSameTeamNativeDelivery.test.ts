...
function teammateBlock(
  overrides: Partial<ParsedTeammateContent> = {}
...
describe('TeamProvisioningSameTeamNativeDelivery', () => {
  ...
  it(
    'persists confirmed native matches, marks them relayed, and consumes fingerprints',
    async () => {
      ...
      expect(markInboxMessagesRead).toHaveBeenCalledWith('alpha', 'lead', [
        { messageId: 'msg-1' },
      ]);
  ...
  it(
    'keeps fingerprints and schedules persist retry when marking confirmed matches read fails',
    async () => {
