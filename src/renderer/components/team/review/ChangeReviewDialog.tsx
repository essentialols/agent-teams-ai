...
function buildUndoDiskMutationSteps(
  actionId: string,
...
export const ChangeReviewDialog = ({
  ...
  const handleUndoRecentDiskAction = useCallback(
    async (action: Extract<ReviewUndoAction, { kind: 'disk' }>): Promise<boolean> => {
      const diskAction = action.action;
      if (
        undoInFlightRef.current ||
        fileApplyInFlightRef.current.has(diskAction.snapshot.filePath)
      ) {
        return false;
      }
      if (!decisionScopeToken) {
      ...
      try {
        ...
        if (diskAction.originalIndex !== undefined) {
          ...
          if (!file) throw new Error('Reviewed file is unavailable for Undo.');
        ...
        const committed = await api.review.executeMutation({
          ...
          kind: 'undo',
          diskSteps: buildUndoDiskMutationSteps(action.id, [diskAction.snapshot]),
          persistedState: {
            hunkDecisions,
            fileDecisions,
            hunkContextHashesByFile: state.hunkContextHashesByFile,
  ...
  const handleUndoLatestReviewAction = useCallback(async (): Promise<void> => {
    ...
    const restored =
      action.kind === 'bulk'
        ...
        : action.kind === 'disk'
          ? await handleUndoRecentDiskAction(action)
