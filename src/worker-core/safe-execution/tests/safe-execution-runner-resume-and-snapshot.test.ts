...
describe("SafeExecutionRunner resumed tasks and snapshots", () => {
  ...
  it(
    ...
    async () => {
      const workspacePath = await gitWorkspace(cleanupPaths, "safe-execution-resume-capacity-");
  ...
  it(
    ...
    async () => {
      const workspacePath = await gitWorkspace(cleanupPaths, "safe-execution-no-continuation-");
      const journal = new InMemoryAttemptJournal();
      let runs = 0;
  ...
  it(
    ...
    async () => {
      const workspacePath = await gitWorkspace(cleanupPaths, "safe-execution-resume-no-continuation-");
      const journal = new InMemoryAttemptJournal();
      const firstRunner = new SafeExecutionRunner({
        lockStore: new InMemoryWorkspaceLockStore(),
        journal,
  ...
  it("does not rerun an interrupted running task with unrecorded workspace changes", async () => {
    const workspacePath = await gitWorkspace(cleanupPaths, "safe-execution-interrupted-dirty-");
    const journal = new InMemoryAttemptJournal();
  ...
  it(
    ...
    async () => {
      const workspacePath = await gitWorkspace(cleanupPaths, "safe-execution-journal-");
      const stateRoot = await tempPath(cleanupPaths, "safe-execution-state-");
  ...
  it("uses only safe read-only git snapshot commands", async () => {
    const workspacePath = await tempPath(cleanupPaths, "safe-execution-git-commands-");
  ...
  it("does not report a full filesystem snapshot as worker-changed files", async () => {
    const workspacePath = await tempPath(cleanupPaths, "safe-execution-filesystem-delta-");
  ...
  it("does not report pre-existing git changes as worker-changed files", async () => {
    const workspacePath = await tempPath(cleanupPaths, "safe-execution-git-delta-");
    const snapshots: WorkspaceSnapshot[] = [
      {
        mode: "git",
        workspacePath,
  ...
  it("uses delta when snapshot modes change during an attempt", async () => {
    const workspacePath = await tempPath(cleanupPaths, "safe-execution-mixed-delta-");
    const snapshots: WorkspaceSnapshot[] = [
      {
        mode: "filesystem",
        workspacePath,
  ...
  it(
    ...
    async () => {
      const repoPath = await tempPath(cleanupPaths, "safe-execution-git-tree-scope-");
      await execFileAsync("git", ["init"], { cwd: repoPath });
      await mkdir(join(repoPath, "app"), { recursive: true });
      await mkdir(join(repoPath, "other"), { recursive: true });
      await writeFile(join(repoPath, "app", "inside.txt"), "base\n", "utf8");
